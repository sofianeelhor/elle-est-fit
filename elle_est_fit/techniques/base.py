...
class TechniqueBase(abc.ABC):
    ...
    def __init__(
        ...
        self.session.headers.update({"User-Agent": "Elle-Est-Fit LFI Framework"})
    ...
    def _default_leak_function(self, filename: str) -> str:
        ...
        if not self.target:
            raise TechniqueError(
                "No target URL provided and no custom leak function defined"
            )
        ...
        # Make the request
        try:
            response = self.session.get(complete_url, timeout=self.timeout)
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"Failed to leak file {filename}: HTTP {response.status_code}"
                )
                return ""
